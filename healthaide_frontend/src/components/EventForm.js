...
class EventForm extends React.Component {
  ...
  componentDidMount () {
    if(this.props.match) {
      ...
      axios({
        method: "GET",
        url: `http://localhost:3000/events/${this.props.match.params.id}`,
  ...
  handleSubmit = e => {
    ...
    const url = this.state.editing ? `http://localhost:3000/events/${this.props.match.params.id}` : 'http://localhost:3000/events'
